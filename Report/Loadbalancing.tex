...
The key objective of a load balancer is connecting clients to servers. There are more than 

\input{figures/tikz/loadBalancerCommunication}
...
\begin{enumerate}
	\item Round robin
	\item Server performance monitoring
	\item Package inspection
\end{enumerate}

Round robin as the name implies distributes connections to servers equally to each server, it does however not automatically result in a fair distribution of traffic since connected clients might not require equal amount of data per connection.
This type has the least overhead but also assumes that data transferred for each connection are almost equal to do a fair distribution.

Server performance monitoring adds overhead by the added network packages and keeping tack of the current performance of the servers. It however ensure that the server with the most free capacity will receive the connection. Free capacity can be defined as free processing time, available memory or another resource.

Package inspection opens up the package and passes higher protocol headers. This could for instance be HTTP headers in order to identify what service the URL is trying to connect to. This allows the system to be split up in smaller specialized parts, having one server deliver aggregated data and another deliver raw data. It also makes it possible to split it up in slow and fast running services, helping to make sure that services which need to be fast are not begin slowed down by services which need to make long calculations.



%Software responsible for handling resource management is refereed to as a load balancer.
%Load balancers can be applied in layers with different responsibility from geographically distributing traffic \todo{ref DNS loadbalncer} to distributing packets to available nodes in a round robin pattern to monitoring nodes and choosing the node with the least load, some even inspect packages to distribute based on package content.

%Load balancers come in different complexity and with the ability to manage resource in various ways.
%From naively assigning clients with a round robin based algorithm to distributing connections based on monitoring node resources like available CPU time. \todo{make ref}
%Some Load balancers require client and or server integration \todo{make ref} while some function completely transparent to network nodes. \todo{make ref}
