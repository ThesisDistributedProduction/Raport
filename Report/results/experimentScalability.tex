...
The regulation cycle time differences of the two systems is illustrated on \cref{fig:timingCentralVSDecentral}. The regulation cycle time of the decentralized solution is defined as the difference between the receive timestamp of the oldest turbine state (tOldestState) received and the timestamp sampled right after, a new turbine state has been sent (tSent):
...
\Cref{fig:exp:cen:turbines} shows how the number of turbines influence the regulation cycle time of the centralized solution. The median values from 5 to 30 turbines is increasing slightly. From 35 turbines and onward the median is linearly increasing.
From 70 turbines onward the outliers take on increasingly extreme values.
The distribution of points at 95 turbines becomes extremely wide for regulation cycle times higher than the median value.
