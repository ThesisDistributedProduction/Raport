...
Looking at Experiment 1, we observe that the regulation cycle time is linearly dependent of the sleep time, with the exception of the 10 ms. The 10 ms sleep time data set can be disregarded for both \cref{subsec:Exper:perfom:1} and \cref{fig:exp:decen:sleep-cache} due to limited experiment equipment. Running with a 10 ms sleep time, caused our experiment setup to approach maximum CPU capacity, thus resulting in package delays.  This is expected since the sleep operation is a part of the regulation cycle. Furthermore we notice negative outliers, implying the regulation cycle time 
