...
DDS relies heavily on the multicast features of the IP protocol described below.
...
Since CFDP does not outperform SDP when using multicast the proposed solutions of this thesis will not use CFDP as their discovery protocols.

\paragraph{A discovery protocol based on Bloom filters} has been proposed as well. Similar to CFDP the Bloom filter approach aims to reduce the number of packets that needs to be exchanged in order to discover all endpoints in the system. The Bloom filter approach introduces a Bloom filter for each DomainParticipant that contains information describing the local endpoints. During the SPDP phase the Bloom filters are sent to all other DomainParticipants. In this way information of all endpoints in the system has been distributed when the SEDP phase begins allowing the DomainParticipants to decrease the number of discovery messages to the number of endpoints that it needs to facilitate communication for its local DataReaders and DataWriters. 
...
Using the Bloom filter approach to enhance the discovery process in DDS is relevant to a system like the one described in the Siemens Case in \cref{sec:SiemensCase}. Despite the advantages of the Bloom filter approach the proposed solutions of this master thesis will not further enhance the discovery protocol of DDS, because the benefits of the enhancement would be present in both the proposed decentralized solution and the proposed centralized solution thus the impact of the enhancement would be unnoticeable when comparing the systems.

\subsubsection{Real Time Publish-Subscribe Protocol}
