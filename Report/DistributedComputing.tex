...
Since the SEDP registers all the DataWriters and DataReaders in the system the protocol scales poorly \cite{(CFDP)}. The discovery time grows as the number of endpoints increase. This affects the scalability of a system directly. To avoid the increase in discovery time a different protocol called Content-based Filtering Discovery Protocol(CFDP) has been proposed. CFDP is similar to SDP during the SPDP phase but it differs in the SEDP phase where changes has been made in order to facilitate content-based discovery of endpoints. The filtering allows the DomainParticipant to reduce the number of discovery messages to the number of endpoints that needs to communicate with the DomainParticipant and its local endpoints. Currently CFDP has only been tested using unicast and here the protocol outperforms SDP in terms of network usage, packages sent and discovery time. However if multicast is used as the underlying transport protocol SDP outperforms CFDP. This happens because SDP only needs one multicast for all DomainParticipants. CFDP in contrast will need one multicast address for each content filter used. This limitation in CFDP is subject for further research. Since CFDP does not outperform SDP when using multicast the proposed solutions of this thesis will use SDP as their discovery protocols.

\subsubsection{Real Time Streaming protocol}
