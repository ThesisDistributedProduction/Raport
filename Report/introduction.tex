...
Siemens Wind Power is increasingly focusing on the offshore wind park market.

The current setup of a wind park requires additional infrastructure for management of the wind park.
As well as the wind turbines the wind park consists of a Wind Power Supervisor for operations that involve the entire park and multiple High Performance Park Pilot, from here on referred to as Park Pilots, for near real-time control of the turbines.
The Wind Power Supervisor and Park Pilots are centralized control points of the wind park, having centralized control means that the system has single points of failure as well, which if it fails decreases the availability of the wind park.
...
Today turbines in a wind park at Siemens are equipped with computers for the purpose of regulating power production parameters, data collection and for communication with the rest of the system. The current wind park setup is illustrated on \cref{fig:currentSiemensSetup}. Every turbine is connected to a Wind Power Supervisor, which is a central component that aggregates data, performs calculations, stores data and handles wind park communication with the outside world. At Siemens up to eight Park Pilots are present pr. wind park and one Wind Power Supervisor.
...
\begin{itemize} 
	...
	\item Can we create a solution that scales so the number of turbines does not impact the regulation cycle time?
	\item Can we create a solution where removing one or more nodes from the system at runtime does not cause system failure?
\end{itemize}
