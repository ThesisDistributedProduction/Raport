...
%	\label{PS:Q:1}
	\item[Feasibility.] \label{PS:Q:CanWeDoIt} How can we best re-implement the current system at Siemens Wind Power (\cref{sec:SiemensCase}) as a system where the Wind Park Supervisor and the Park Pilots are decentralized?
%	\label{PS:Q:4}
	\item[Availability.] \label{PS:Q:Availlability} Can we create a solution where removing one or more nodes from the system at runtime does not cause system failure?
%	\label{PS:Q:3}
	\item[Performance.] \label{PS:Q:CanWeScale} Can we create a solution that scales so the number of turbines does not impact the regulation cycle time?
%	\item Can we, with the new decentralized solution, reduce the current regulation cycle time of 150 ms?
%	\label{PS:Q:2}
	\item[Scalability.] \label{PS:Q:HowWellDoWeScale} How will the new decentralized system impact the following parameters compared to the current centralized system at Siemens Wind Power:
	\begin{enumerate}
...
%afgrænsning til HPPPen og regulerings sekvensen.
