...
For every transformer station there is a High Performance Park Pilot from here on refereed to as Park Pilot. The Park Pilots are responsible for park regulation. This is done by monitoring all the turbines performance parameters and issue correct set points for power production.
...
\begin{itemize} 
	\item Single point of failure. Should a Wind Power Supervisor or a Park Pilot fail, a part of the windmill farm will become unavailable.
	\item Low scalability. The Wind Power Supervisors and Park Pilots does not scale with the number of windmills. This is reflected in the park regulation performance. Today a park regulation cycle-time is 150 ms. This time is set after worst case number of turbines, network overhead and network congestions pr. Park Pilot. Every turbine needs to return their current status to the Park Pilot, the set points for each turbine is calculated in the Park Pilots and finally the new set points can be pushed to the turbines, as illustrated on \cref{fig:dataComputationSequence}. 
\end{itemize}

Siemens wishes to decentralize the Wind Power Supervisor and Park Pilots by distributing their functionality to the turbines, utilizing the free capacity of the computers already residing in every turbines. This would increase availability by lowering the possibility of single point of failure and increase scalability, which would make Siemens able to lower the park regulation cycle-time. 
