...
The IDL definitions are defined from looking at what data to transfer between the turbines and the Park Pilot. \Cref{fig:cenTurbineDataMessage} for example is the IDL definition of the data transfer object used as reply from the turbines to the Park Pilot. 
...
The profile from the current Siemens system QoS XML file chosen for the centralized solution is \textit{SwpStrictReliableNoDurability} (\cref{appendix:siemensQosFile}).
...
\begin{table}
	\begin{tabular}{l l l}
		...
		\hline
		Wind Park Supervisor & \checkmark & \text{x} \\
		\hline
		\hline
	\end{tabular}
