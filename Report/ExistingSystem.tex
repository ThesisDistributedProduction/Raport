...
\subsection{Choosing quality of service parameters}
...
The Siemens QoS XML file specifies two \textit{DomainParticipant} profiles. One using UDPv4 and one using DTLS as transport plugin. The one using UDPv4 has been chosen for the centralized solution, since DTLS encryption and decryption is an unnecessary overhead, that should either be introduced to both the centralized- and the decentralized solution or to neither, for the purpose of this thesis. 
...
All parameters of the quality of service configuration for the centralized solution is presented and evaluated in section \cref{sec:detailedQoSDesc}.
...
\begin{itemize}
	...
	\item \textbf{multicast\_enabled} is set to 0 which means multicast is disabled for the centralized solution. 
	\item \textbf{message\_size\_max} the maximum size of a message in bytes. This value must be set before the transport plugin is registered, so that \textit{Connext} can properly use the plugin.
	\item \textbf{send\_socket\_buffer\_size} the size of the send buffer. 
	\item \textbf{recv\_socket\_buffer\_size} 
\end{itemize}
...
\paragraph{Topics} (\cref{fig:parQos}) QoS parameters has been evaluated as follows:
...
\end{lstlisting}
\caption[Topic QoS parameters]{
		\label{fig:topicQos} 
		\footnotesize{Topic QoS parameters.}
	}
\end{figure}
